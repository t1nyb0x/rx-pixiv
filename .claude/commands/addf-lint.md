...
| 9. `.claude/addf/addfTools/migrate-paths.py` ⇔ `.claude/addf/addfTools/lint-residual-paths.py` | `compile_pattern` 同期ブロック（`# --- BEGIN/END sync: compile_pattern (Plan 0068) ---`）の (a) 正規化テキスト一致、(b) 必須シンボル存在検査（`compile_pattern` / `BoundaryPattern` / `_self_url_prefixes`）、(c) 順序ドリフト明示。Plan 0068 で導入した URL スキーム検出設計の同期契約 | WARNING |
