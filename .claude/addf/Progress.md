...
##### 2026-08-10 — Plan 0006 完了（NSFW ゲート）

**やったこと**:
`NsfwPolicy` / `channelRating` / `ImageUrlRewriter` / `MediaSelector` を実装。
テスト323件全通過、文 96.3% / 分岐 88.33%（分岐は久々に改善した）。

**Plan の記述に誤りを見つけて訂正した。**
Plan 0006 には「`i.pximg.net` 以外のホストは書き換えず、埋め込みもしない」と書いてあった。
**そのまま実装すると phixiv 経路と OGP 経路の画像がすべて消える。**
phixiv は既にプロキシ済みの `https://phixiv.net/i/...` を返し、
OGP は `https://embed.pixiv.net/...` を返すため。
この Plan は項目3・4 の実測より前に書かれていた —— **計画が現実に追い越された**形。
正しくは「i.pximg.net は書き換え、埋め込み可能と実測できたホストは素通し、それ以外は弾く」。

**テストの書き方で意識したこと**:
54ケースの期待値表は**実装から導かず、ADR 0006 の判定表を人手で書き写した**。
実装と同じ計算をテストに書くと、テストが実装の写経になって意味を失うから。
加えて「通常チャンネルで制限付き作品のメディアが出ることは無い」という
**表とは独立した不変条件**を別テストに置いた。表を書き間違えてもこれが守る。

**今の見立て**:
Plan 0006 は完了。項目6（添付フォールバック `PximgFetcher`）は
Plan 自身が「既定が無効なので v1 の完了条件から外す」としているため未実装。

**次の自分へ**:
Plan 0011（管理コマンド・Redis）か Plan 0007（レンダリング配線）。
TODO の並列化メモどおり **0011 を先に片付けるほうが `MessageHandler` の衝突が小さい**。

**気になっていること**:
- `channelRating` は discord.js の `ChannelType` に依存する。
  Discord が新しいチャンネル型を追加したとき、既定で「年齢制限チャンネルではない」に
  倒れる実装にしてあるが、それが**安全側とは限らない**（新型が実質 NSFW でも展開しうる）。
  ただし現状これ以上の手当ては難しい
- `filterByRating` はジェネリックで書いたので `UserRecentWork` 以外にも使える。
  Plan 0007 で最近作サムネイルに実際に配線すること

##### 2026-08-10 — Plan 0011 の純粋部分を実装

**やったこと**:
`AccessGate` / `parseOwnerCommand` / インメモリ実装3種。テスト368件全通過、
文 96.61% / 分岐 89.46%（分岐は改善が続いている）。

**ゲートの順序が設計そのもの**なので、順序と縮退をテストで固定した:

1. 禁止（利用者・サーバー）→ 2. 許可リスト → 3. クールダウン
   （ここまでが URL 検出より前。濫用時に本文解析すらさせない）
4. 展開拒否リストは**取得の前**（拒否対象に pixiv へリクエストを飛ばさない）

縮退は ADR 0016 どおり非対称にした:
- **禁止・展開拒否を読めない → 止まる**（ban が効いているか分からない状態で動かない）
- **クールダウンを読めない → 通す**（安全側の判定に影響しないため）

コマンド解釈は discord.js に依存しない純粋関数にした。
全コマンドをテーブルテストで担保でき、「通常の会話をコマンドとして解釈しない」
「未知のコマンドを黙って無視しない」も固定できる。

**今の見立て**:
Plan 0011 は一部完了。残りは Redis 実装（`redis` パッケージの追加が要る）と
`OwnerCommandHandler` の discord.js 配線。
インメモリ実装があるので、**Redis なしでも動く状態にはなっている**
（ただし再起動で禁止が消えるため本番運用はできない）。

**次の自分へ**:
Redis 実装を入れるか、Plan 0007（レンダリング配線）へ進むか。
`AccessGate` はポート越しなので、Redis 実装を後から差し込んでも
ゲートのロジックは1行も変わらない。

**気になっていること**:
- `AccessGate` の縮退テストで、ストア障害を Proxy で作った。
  実際の Redis クライアントが投げる例外の形が違う可能性がある
- 展開拒否の「pixiv ユーザー ID による一括ブロック」は、
  作品の作者 ID が分かって初めて効く。取得前の判定では作品 ID しか照合できない。
  **作者単位のブロックは取得後にもう一度照合する必要がある** —— Plan 0007 で配線すること

##### 2026-08-11 — Redis 実装。テストが起動ブロックのバグを捕まえた

**やったこと**:
`redis@5` を追加し、`RedisConnection` / `RedisBanRepository` / `RedisBlockRepository` /
`RedisCooldownStore` を実装。テスト383件全通過、文 95.94% / 分岐 89.42%。

**テストが実装のバグを1件捕まえた。**
「Redis に繋がらなくても起動する」という ADR 0016 の契約をテストに書いたところ、
**`connect()` が解決せずタイムアウトした**。原因は再接続戦略を設定したこと ——
`redis` クライアントは再試行を続け、`connect()` の Promise を解決しない。
そのまま `await` すると**起動が永久にブロックされる**。

直し方: 初回接続は予算（既定3秒）まで待ち、**再試行はバックグラウンドへ流す**。
`Promise.race` で予算と競争させ、負けたら false を返す。
これで「起動は止まらない」と「復旧は自動で検知する」の両方が成り立つ。

契約をテストに書かなければ、**本番で初めて分かる**類のバグだった。

**設計判断を2つ**:

1. **禁止・展開拒否はプリロード方式**（ADR 0016 どおり）。
   起動時に全件をプロセス内へ読み、以後は Redis に往復しない。
   **プリロード前の参照は例外にした** —— 空を返すと「禁止されていない」と
   誤読され、ゲートが素通しになるため
2. **クールダウンはプリロードしない**。件数が多く寿命も短いので
   `SET NX PX` に判定ごと任せる。判定と記録が1往復で済む

**次の自分へ**:
Plan 0011 の残りは `OwnerCommandHandler` の discord.js 配線のみ。
その後 Plan 0007（レンダリング配線）。

**気になっていること**:
- `RedisConnection` のテストは実ネットワーク（127.0.0.1:1）に触れる。
