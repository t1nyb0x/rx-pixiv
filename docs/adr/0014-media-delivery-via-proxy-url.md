...
| 3 | 添付方式へのフォールバック（ADR 0005 の方式） | 将来候補。現在は未実装・設定入口なし |

第3段を実装する場合は `IMediaFetcher` の別実装と設定入口を同じPlanで追加する。
...
- `PximgFetcher`（バイト取得）— 第3段の将来候補であり、現在は実装しない
...
- Botは画像バイトを取得しないため、画像プロキシの実取得成否は直接観測できない。
  `pixiv_media_total{result}` はURL選択・書換えのselected/rejected/omittedを記録し、
  proxy可用性を監視する場合は将来の明示的probeとして別途設計する
