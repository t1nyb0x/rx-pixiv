...
- **`not_found`（HTTP 404 等）のときのみ連鎖を*失敗として*打ち切る。** それ以外の失敗は次段へ進む
  （成功しても「十分」になれば打ち切る。下記「打ち切り条件」を参照）
...
### 補完マージ —— 「最初に成功した経路を採用」では足りない

**2026-08-10 追記**（Plan 0005 項目3・4 の実測を受けて）。

当初は「最初に成功した経路の結果を返す」という素朴な連鎖を想定していた。
しかし実測により、**R-18 作品では経路ごとに持っている情報が異なる**ことが判明した。

| 経路 | 年齢区分 | 画像 |
|---|---|---|
| Ajax | **`authoritative`**（`xRestrict`） | **無し**（`/pages` が 404） |
| phixiv | `inferred`（タグ由来） | **有り** |
| OGP | `inferred`（プレビューの有無） | 無し（ロゴに差し替わる） |

素朴な連鎖では Ajax が最初に成功して打ち切られ、**R-18 の画像が永久に出ない**。
逆に phixiv を先に置くと、年齢区分が `inferred` に落ちて
[ADR 0006](0006-age-restricted-content.md) のフェイルクローズにより通常チャンネルで展開されなくなる。

したがって連鎖は**各段の結果を蓄積して補完する**。

| 項目 | マージ規則 |
|---|---|
| 年齢区分 | `escalateRating` で**制限を強める方向にのみ**畳む。確信度は高いほうを採る |
| 画像 | 蓄積側が空なら後段のもので**埋める** |
| その他のメタデータ | **先に取れたものを優先**（＝権威ある経路を尊重）。欠けている項目だけ後段で埋める |
| 種別が食い違う場合 | 蓄積側を採り、年齢区分だけ畳む |

### 打ち切り条件

すべての経路を常に叩くのは無駄なので、**「十分」になった時点で止める**。

- **イラスト**: 年齢区分が `authoritative` かつ画像が1枚以上 → 打ち切る
- **それ以外**（小説・シリーズ・ユーザー）: 最初の成功で打ち切る
  （画像の欠落が起きるのはイラスト経路だけであるため）

この条件により、全年齢作品は Ajax の1回で完結し、
R-18 作品のときだけ phixiv へ進む。
